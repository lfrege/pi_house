...
*/
	...
	this.scalePercent = function(point)
	{
		var min, max;
		var i;
		...
		return { x: threeView((new Date(point.created).getTime() - this.min_x) * this.width / (this.max_x - this.min_x)),
			 load1: threeView((1-point.load1) * this.height),
			 load2: threeView((1-point.load2) * this.height),
			 load3: threeView((1-point.load3) * this.height),
			 freemem: threeView((this.totalmem-point.freemem) * this.height / this.totalmem)
			};
	}
	...
	this.canvasString = function()
	{
		...
		for (i = 0; i < this.points.length; i++)
		{
			load1array.push(this.scalePercent(this.points[i]));
		}

		output += "<canvas id='" + this.hostname + "Chart' width=" + (this.width + this.leftborder) + " height=" + (this.height + this.bottomborder) + "></canvas>\n";
		output += "<script type='text/javascript'>\n"
			+ "drawData('" + this.hostname + "Chart', "
			+ JSON.stringify(load1array) + ", " + this.leftborder
			+ ", " + this.bottomborder + ");\n"
			+ "</script>\n";
		return output;
	}
