...
const read_ping_today = "SELECT ipaddr, platform, hostname, totalmem, load1, load2, load3, freemem, ping.created "
	+ "FROM hosts, ping "
	+ "WHERE hosts.host_id=ping.host_id "
	+ "AND DATE_SUB(CURDATE(), INTERVAL 1 DAY) <= ping.created "
	+ "ORDER BY hostname, ping.created ";
...
getPingToday = function(callback)
{
	var connection = mysql.createConnection({host: server, user: user, password: password, database:database});

	connection.query(read_ping_today, callback);
...
module.exports.getPingToday = getPingToday;
